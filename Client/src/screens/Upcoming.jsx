...
const Upcoming = ({ reservation }) => {
    ...
    const fetchUpcoming = async () => {
        ...
        try {
            const { data } = await axios.get(`http://192.168.1.184:3000/api/reservations/upcoming/${customer.id}`)
    ...
    const getName = async (restaurantID) => {

        try {
            const { data } = await axios.get(`http://192.168.1.184:3000/api/restaurants/${restaurantID}`)
            return data
        } catch (error) {
            console.log(error)
        }
    ...
    return (
        <View style={styles.container}>
            <ScrollView vertical style={{ marginTop: -80 }} >
                {upcomingReservations.map((reservation) => (
                    <View key={reservation.id} style={{
                        ...
                        <Text style={[styles.pm, styles.rosemarysTypo]}>{moment(reservation.time).format('LT') + ' z ' + reservation.id}</Text>
                    </View>
                ))}
